...
export class SignupPage extends AbstractPage
{
	...
	async selectTitle(decision: string)
	{
		if (decision === constants.mrOption)
		{
			await this.title1stOptionLabel.check();
		}
		else if (decision === constants.mrsOption)
		{
			await this.title2ndOptionLabel.check();
		}
		else
		{
	...
	async provideCompany(company: string)
	{
